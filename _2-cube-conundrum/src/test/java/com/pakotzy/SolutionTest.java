...
class SolutionTest {
    private final Solution solution = new Solution(12, 13, 14);

    @Test
    void sumPowerLeastValidSetsWhenProblem() throws IOException {
        long sumOfPower = solution.sumPowerLeastValidSets(solution.compileGames(ConsoleReader.readToString(new String[]{getProblemInput()}, null)));
        assertEquals(72706, sumOfPower);
    }

    @Test
    void sumPowerLeastValidSetsWhenExample() throws IOException {
        long sumOfPower = solution.sumPowerLeastValidSets(solution.compileGames(ConsoleReader.readToString(new String[]{getExampleInput()}, null)));
        assertEquals(2286, sumOfPower);
    }

    @Test
    void sumValidGamesWhenExample() throws IOException {
        long sumOfId = solution.sumValidGames(solution.compileGames(ConsoleReader.readToString(new String[]{getExampleInput()}, null)));
        assertEquals(8, sumOfId);
    }

    @Test
    void sumValidGamesWhenProblem() throws IOException {
        long sumOfId = solution.sumValidGames(solution.compileGames(ConsoleReader.readToString(new String[]{getProblemInput()}, null)));
        assertEquals(1853, sumOfId);
    }

    @Test
    void compileGamesSingleLine() {
        String input = "Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green";
        List<Game> games = solution.compileGames(Collections.singletonList(input));

        assertEquals(1, games.size());
        assertEquals(3, games.get(0).sets().size());

        assertEquals(0, games.get(0).sets().get(0).red());
        assertEquals(2, games.get(0).sets().get(0).green());
        assertEquals(0, games.get(0).sets().get(0).blue());

        assertEquals(1, games.get(0).sets().get(1).red());
        assertEquals(2, games.get(0).sets().get(1).green());
        assertEquals(6, games.get(0).sets().get(1).blue());

        assertEquals(4, games.get(0).sets().get(2).red());
        assertEquals(0, games.get(0).sets().get(2).green());
        assertEquals(3, games.get(0).sets().get(2).blue());
    }

    @Test
    void compileGamesMultiLine() {
        List<String> input = Arrays.asList(getExampleInput().split("\n"));
        List<Game> games = solution.compileGames(input);

        assertEquals(5, games.size());

        assertEquals(18, games.get(0).sets().stream().mapToInt(set -> set.red() + set.green() + set.blue()).sum());
        assertEquals(13, games.get(1).sets().stream().mapToInt(set -> set.red() + set.green() + set.blue()).sum());
        assertEquals(62, games.get(2).sets().stream().mapToInt(set -> set.red() + set.green() + set.blue()).sum());
        assertEquals(51, games.get(3).sets().stream().mapToInt(set -> set.red() + set.green() + set.blue()).sum());
        assertEquals(15, games.get(4).sets().stream().mapToInt(set -> set.red() + set.green() + set.blue()).sum());
    }
